...
export default {
  ...
  methods: {
    switchToLogin() {
      this.$root.$emit("open-yattlogindialog"); // CTODO - do we want to add this?
      this.$root.$emit("close");
    },
    async handleConfirm() {
      await yattIntegrationHelper.updateProfile(this.credentials, this.user);
      this.$root.$emit("close");
      // CTODO emit snackbar message and also handle errors
    },
    handleClose() {
      this.$root.$emit("close");
    },
