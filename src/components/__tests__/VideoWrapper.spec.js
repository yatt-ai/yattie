...
describe("VideoWrapper.vue", () => {
  ...
  test('triggers a click of "Cancel" button', async () => {
    const wrapper = mount(VideoWrapper, {
      propsData: {
        item: itemData,
      },
    ...
    expect(
      wrapper.find(".video-control .text-capitalize:nth-child(1)")
    ).toBeTruthy();
    expect(
      wrapper.find(".video-control .text-capitalize:nth-child(1)").text()
    ).toContain("Cancel");

    const button = wrapper.find(".video-control .text-capitalize:nth-child(1)");
    const event = jest.fn();
  ...
  test('triggers a click of "Apply" button', async () => {
    const wrapper = mount(VideoWrapper, {
      propsData: {
        item: itemData,
      },
    ...
    expect(
      wrapper.find(".video-control .text-capitalize:nth-child(2)")
    ).toBeTruthy();
    expect(
      wrapper.find(".video-control .text-capitalize:nth-child(2)").text()
    ).toContain("Apply");

    const button = wrapper.find(".video-control .text-capitalize:nth-child(2)");
    const event = jest.fn();
